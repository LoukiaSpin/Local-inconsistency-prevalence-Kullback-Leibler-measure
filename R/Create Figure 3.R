...
kld_diff_single <-
  ...
  geom_vline(xintercept = 0,
             linetype = 2) +
  geom_point(aes(colour = node_standard,
  ...
  labs(x = "Posterior mean of inconsistency factor",
       ...
       colour = "Inconsistency based on 95% CrI of inconsistency factor") + 
...
kld_diff_sd_single <-
  ...
  geom_point(aes(colour = node_standard,
             ...
             shape = 21,
             stroke = 2) +  
  labs(x = "Posterior standard deviation of inconsistency factor",
       y =  expression(bold(paste("Interpretation index ", D^j))),
